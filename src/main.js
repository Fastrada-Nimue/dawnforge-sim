...
// ============== TEST SYNERGY PANEL ==============
function initTestSynergyPanel() {
  const testBtn = document.getElementById("test-synergy-btn");
  const testPanel = document.getElementById("test-synergy-panel");
  ...
  // Populate synergy options
  const pairContainer = document.getElementById("test-pair-options");
  const tripleContainer = document.getElementById("test-triple-options");
  const apexContainer = document.getElementById("test-apex-options");

  if (pairContainer) {
    for (const def of COMBO_DEFS) {
      const btn = document.createElement("button");
      btn.className = "test-synergy-option";
      btn.textContent = capitalize(def.key);
      btn.addEventListener("click", () => testUnlockAndCastCombo(def.key, "pair"));
  ...
  if (tripleContainer) {
    for (const def of TRIPLE_COMBO_DEFS) {
      ...
      btn.className = "test-synergy-option";
      btn.textContent = capitalize(def.key);
      btn.addEventListener("click", () => testUnlockAndCastCombo(def.key, "triple"));
  ...
  if (apexContainer) {
    for (const def of APEX_COMBO_DEFS) {
      ...
      btn.className = "test-synergy-option";
      btn.textContent = capitalize(def.key);
      btn.addEventListener("click", () => testUnlockAndCastCombo(def.key, "apex"));
      apexContainer.appendChild(btn);
    }
  }
}

function testUnlockAndCastCombo(key, type) {
  if (!game || game.phase !== "run") {
    console.warn("Must be in a run to test synergies");
    return;
  }

  let def = null;
  if (type === "pair") def = COMBO_DEFS.find(d => d.key === key);
  else if (type === "triple") def = TRIPLE_COMBO_DEFS.find(d => d.key === key);
  else if (type === "apex") def = APEX_COMBO_DEFS.find(d => d.key === key);

  if (!def) return;

  // Unlock base elements if needed
  const elements = type === "apex" ? [def.a, def.b, def.c, def.d] : type === "triple" ? [def.a, def.b, def.c] : [def.a, def.b];
  for (const elem of elements) {
    if (game.powers[elem] && !game.powers[elem].unlocked) {
      unlockPower(elem, false);
    }
    if (game.powers[elem]) {
      game.powers[elem].level = Math.max(game.powers[elem].level, def.minLevel);
    }
  }

  // Unlock combo
  if (type === "pair") {
    game.pairFusionUnlocked[key] = true;
  } else if (type === "triple") {
    game.tripleFusionUnlocked[key] = true;
  } else if (type === "apex") {
    game.apexFusionUnlocked[key] = true;
  }

  // Cast it at center of arena
  game.selectedCastKey = key;
  const centerX = canvas.width / 2;
  const centerY = canvas.height * 0.6;
  tryCastPower(key, { x: centerX, y: centerY });
